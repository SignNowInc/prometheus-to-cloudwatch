...
var (
	...
	listenAddress               = flag.String("listen_address", os.Getenv("LISTEN_ADDRESS"), fmt.Sprintf("Address to expose metrics (default: %s)", DefaultListenAddress))
	metricsPath                 = flag.String("metrics_path", os.Getenv("METRICS_PATH"), fmt.Sprintf("Path under which to expose metrics (default: %s)", DefaultMetricPath))
)
...
func startHttpServer(shutdownWaiter *sync.WaitGroup) *http.Server {
	...
	if metricsListenAddress == "" {
		metricsListenAddress = DefaultListenAddress
	}

	var metricsListenPath = *metricsPath
	if metricsListenPath == "" {
		metricsListenPath = DefaultMetricPath
	}
